...
static inline void hdd_update_pm_qos_affine_cores(struct hdd_context *hdd_ctx)
{
	...
	hdd_ctx->pm_qos_req.cpus_affine = 15;
}
#else
static inline void hdd_update_pm_qos_affine_cores(struct hdd_context *hdd_ctx)
{
					  ...
					  const uint64_t rx_packets)
{
	...
	unsigned int silver_mask = 15;
	unsigned int gold_mask = 127;
	...
	pm_qos_cpu_mask = silver_mask;
	if (is_rx_pm_qos_high)
		pm_qos_cpu_mask = gold_mask;
